...
class Carrito {
    constructor() {
        this.articulos = []; // Array con los artículos
    }
    ...
    add(producto) {
        ...
        if (existe) {
            ...
            // Uso una copia para no modificar el objeto original del listado, vaya a liarla
    ...
    dibujaCarrito() {
        ...
        h2.textContent = `Carrito (${this.articulos.length} productos distintos)`;
        ...
        for (let i = 0; i < this.articulos.length; i++) {
                numtotal = numtotal + this.articulos[i].cantidad;
            };
            total.textContent = `Cantidad total de productos añadidos al carrito: ${numtotal}`;
    ...
    eliminar(producto){
        this.articulos.splice(this.articulos.findIndex(p => p.id === producto.id), 1);
...
function CreaCard(objProducto) {
    ...
    icono.addEventListener('click', () => {
        ...
        miCarrito.eliminar(objProducto);
    })
...
// DevuelveGeneros(arObjBDProductos): Devolverá un array con las categorias de los productos sin repetirse
